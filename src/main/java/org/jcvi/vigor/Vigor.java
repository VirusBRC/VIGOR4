...
import org.jcvi.vigor.service.GeneModelGenerationService;
...
import org.jcvi.vigor.utils.GenerateAlignmentOuput;
...
public class Vigor {
	...
	public void run(String... args) {
		...
		try {
			...
			if (parsedArgs.getBoolean(CommandLineParameters.listDatabases)) {
				...
				VigorUtils.checkFilePath("reference database path", referenceDatabasePath, VigorUtils.FileCheck.EXISTS,
						VigorUtils.FileCheck.DIRECTORY, VigorUtils.FileCheck.READ);
				List<VigorInitializationService.DatabaseInfo> databases = initializationService
						.getDatabaseInfo(referenceDatabasePath);
		...
		for (VigorInitializationService.DatabaseInfo db : sortedDatabases) {
			...
			if (db.configFile.isPresent()) {
				try {
					...
					VigorConfiguration vigorConfiguration = initializationService.mergeConfigurations(
							initializationService.loadVirusConfiguration(db.configFile.get().getAbsoluteFile()));
					if (vigorConfiguration.hasSection(VigorConfiguration.METADATA_SECTION)) {
						String virusName = vigorConfiguration.get(VigorConfiguration.METADATA_SECTION,
								ConfigurationParameters.VirusName);
						if (!NullUtil.isNullOrEmpty(virusName)) {
							dbInfo.add(String.format("  Virus: %s", virusName));
						}
						List<String> aliases = vigorConfiguration.get(VigorConfiguration.METADATA_SECTION,
								ConfigurationParameters.Alias);
						if (!(aliases == null || aliases.isEmpty())) {
							dbInfo.add(String.format("  Alias(es): %s", String.join(", ", aliases)));
						}
						String description = vigorConfiguration.get(VigorConfiguration.METADATA_SECTION,
						...
						List<String> notes = vigorConfiguration.get(VigorConfiguration.METADATA_SECTION,
								ConfigurationParameters.Note);
	...
	private void checkConfig(VigorConfiguration config) throws VigorException {
		...
		if (!NullUtil.isNullOrEmpty(outputDirectoryPath)) {
			...
			if (!(outputDirectory.exists() && outputDirectory.isDirectory())) {
				errors.add(String.format("Invalid output prefix %s/%s. Please provide a directory followed by a file prefix",
		...
		try {
			VigorUtils.checkFilePath("Reference database file", reference_db, VigorUtils.FileCheck.EXISTS,
					VigorUtils.FileCheck.FILE, VigorUtils.FileCheck.READ);
		} catch (VigorException e) {
		...
		if (tempDir.exists()) {
			try {
				VigorUtils.checkFilePath("temporary directory", temporaryDirectory, VigorUtils.FileCheck.DIRECTORY,
						VigorUtils.FileCheck.READ, VigorUtils.FileCheck.WRITE);
			} catch (VigorException e) {
				errors.add(e.getMessage());
			}
		} else {
	...
	private void printConfiguration(VigorConfiguration vigorParameters) {
		...
		LOGGER.info(
				() -> vigorParameters.entrySet().stream()
						.sorted(Comparator.comparing(es -> es.getKey().configKey, String.CASE_INSENSITIVE_ORDER))
						.map(e -> String.format("%-50s%s (%s)", e.getKey().configKey, e.getValue(),
								vigorParameters.getWithSource(e.getKey()).orElse(unset).source))
				.collect(Collectors.joining("\n")));
	}

	public void generateAnnotations(String inputFileName, String referenceDB, VigorConfiguration vigorParameters)
			throws VigorException {
		try {
		...
		VigorUtils.checkFilePath("output directory", outputDir, VigorUtils.FileCheck.EXISTS, VigorUtils.FileCheck.WRITE,
		...
		try (NucleotideFastaDataStore dataStore = new NucleotideFastaFileDataStoreBuilder(new File(inputFileName))
				...
				GenerateVigorOutput.Outfiles outfiles = getOutfiles(outputDir, outputPrefix,
	...
	public void outputModels(VigorConfiguration vigorParameters, GenerateVigorOutput.Outfiles outfiles, List<Model> geneModels)
			throws IOException {
	...
	public List<Model> modelsFromNucleotideRecord(NucleotideFastaRecord record, String referenceDB,
			VigorConfiguration vigorParameters) throws VigorException {
		...
		LOGGER.debug("Found {} peptides for {} models for sequence {}",
				geneModels.stream().map(m -> m.getMaturePeptides().size()).reduce(Integer::sum).orElse(0), geneModels.size(),
				record.getId());

		// sort by begin,end
		return geneModels.stream()
				.sorted(Comparator.comparing(
						m -> VigorFunctionalUtils.getDirectionBasedRange(m.getRange(),
								m.getAlignment().getVirusGenome().getSequence().getLength(), m.getDirection()),
				Range.Comparators.ARRIVAL)).collect(Collectors.toList());

	}

	/**
	 * Handle alignments on the opposite strand by reverse complementing the
	 * sequence and recalculating stops and gaps
	 * 
	...
	private List<Alignment> handleReverseAlignments(VigorConfiguration vigorParameters, List<Alignment> alignments) {
		for (Alignment alignment : alignments) {
			if (alignment.getDirection() == Direction.REVERSE) {
				VirusGenome complement = VirusGenomeService.reverseComplementVirusGenome(alignment.getVirusGenome(),
						vigorParameters);
	...
	private List<Model> findPeptides(VigorConfiguration config, List<Model> geneModels) throws VigorException {
		...
		for (Model model : geneModels) {
			...
			if (!(maturePeptideDB == null || maturePeptideDB.isEmpty())) {
				...
				model.setMaturePeptides(peptideMatchingService.findPeptides(model, new File(maturePeptideDB), scores));
	...
	public List<Alignment> generateAlignments(VirusGenome genome, String referenceDB, VigorConfiguration configuration)
			throws VigorException {
	...
	public void generateOutput(VigorConfiguration config, List<Model> models, GenerateVigorOutput.Outfiles outfiles)
			throws IOException {

		generateVigorOutput.generateOutputFiles(config, outfiles, models);
	}

	public void generateGFF3Output(String outputDir, List<Model> models, GenerateVigorOutput.Outfiles outfiles)
			throws IOException {
	...
	public void generateAlignmentOutput(String outputDir, List<Model> models, GenerateVigorOutput.Outfiles outfiles) {
	...
	private GenerateVigorOutput.Outfiles getOutfiles(String outputDir, String outputPrefix, boolean overwrite)
			throws IOException {
	...
	private void writeEffectiveConfig(String outputDir, String outputPrefix, VigorConfiguration configuration)
			throws IOException {

		String dateString = new SimpleDateFormat("yyyyMMdd-HHmmss").format(new Date());
		try (BufferedWriter writer = Files.newBufferedWriter(
				Paths.get(outputDir, String.format("%s-%s.ini", outputPrefix, dateString)), Charset.forName("UTF-8"),
				StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING)) {
			writer.write(String.format("; Effective configuration %s\n\n", dateString));
			VigorConfiguration.ValueWithSource val;
			List<ConfigurationParameters> parameters = configuration.keySet().stream()
					.sorted(Comparator.comparing(p -> p.configKey, String.CASE_INSENSITIVE_ORDER)).collect(Collectors.toList());
	...
	public void initiateReportFile(String outputDir, String outputPrefix, int verbose) {
		...
		FileAppender fa = FileAppender.newBuilder().withName("mylogger").withAppend(false)
				.withFileName(new File(outputDir, outputPrefix + ".rpt").toString()).build();
		fa.start();
		config.addAppender(fa);

		Layout warningLayout = PatternLayout.newBuilder().withConfiguration(config).withPattern("%level %msg %exception{full}\n")
				.build();

		Filter warningFilter = LevelRangeFilter.createFilter(Level.FATAL, Level.WARN, Filter.Result.ACCEPT, Filter.Result.DENY);

		FileAppender warnings = FileAppender.newBuilder().withName("__warnings").withAppend(false).withLayout(warningLayout)
				.withFileName(new File(outputDir, outputPrefix + ".warnings").toString()).build();
		warningFilter.start();
